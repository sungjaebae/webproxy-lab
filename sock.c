...
// 시그널에 마스크 set을 설정하고 프로세스를 시그널 대기상태로 만든다.
...
// long을 base가 b인 문자열로 변환 (from K&R)
...
static ssize_t rio_read(rio_t *rp, char *usrbuf, size_t n) {
  ...
  while (rp->rio_cnt <= 0) { /* Refill if buf is empty */
    ...
    if (rp->rio_cnt < 0) {
      ...
      rp->rio_bufptr = rp->rio_buf; /* Reset buffer ptr, 루프 다시돈다 */
  }
 ...
 **********************************/
ssize_t Rio_readn(int fd, void *ptr, ssize_t nbytes) {
  ssize_t n;
...
void Rio_writen(int fd, void *usrbuf, ssize_t n) {
